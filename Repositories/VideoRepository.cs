    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                          SELECT v.Title, v.Description, v.Url, v.DateCreated, 
                          v.UserProfileId, 
                          up.Name, up.Email, up.DateCreated AS UserProfileDateCreated, 
                          up.ImageUrl AS UserProfileImg, up.Id
                          FROM Video v
                          JOIN UserProfile up ON v.UserProfileId = up.Id
                        ...
                        if (reader.Read())
                        {
                            video = new Video()
                            {
                                ...
                                UserProfile = new UserProfile()
                                {
                                    ...
                                    Email = DbUtils.GetString(reader, "Email"),
                                    DateCreated = DbUtils.GetDateTime(reader, "DateCreated"),
                                    ImageUrl = DbUtils.GetString(reader, "UserProfileImg")
        ...
        public Video GetVideoByIdWithComments(int Id)
        {
            using (var conn = Connection)
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"SELECT v.Id, v.Title, v.Description, v.Url, v.DateCreated, v.UserProfileId, up.Id AS
                                        VideoUserProfileId,
                                        up.Name, up.Email, up.DateCreated AS UserProfileDateCreated,
